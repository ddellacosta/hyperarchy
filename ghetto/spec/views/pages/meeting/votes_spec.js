...
describe("Views.Pages.Meeting.Votes", function() {
  var votesView, meeting, currentUser, otherUser, currentUserVote, otherUserVote, votesRelation;

  beforeEach(function() {
    attachLayout();
    votesView = Application.meetingPage.votes;

    meeting = Meeting.createFromRemote({id: 1});
    currentUser = User.createFromRemote({id: 1});
    otherUser = User.createFromRemote({id: 2});

    currentUserVote = currentUser.votes().createFromRemote({id: 1, meetingId: meeting.id(), updatedAt: 1308353647242});
    otherUserVote = otherUser.votes().createFromRemote({id: 2, meetingId: meeting.id(), updatedAt: 1308353647242});

    votesRelation = meeting.votes();
  });
  ...
  describe("when votes are added and removed", function() {
    it("adjusts the vote count in the header accordingly", function() {
      ...
      currentUser.votes().createFromRemote({id: 1, meetingId: meeting.id(), updatedAt: 1308353647000});
      expect(votesView.header.html()).toEqual('1 Vote');

     otherUser.votes().createFromRemote({id: 2, meetingId: meeting.id(), updatedAt: 1308353647001});
     expect(votesView.header.html()).toEqual('2 Votes');
    });
  });
