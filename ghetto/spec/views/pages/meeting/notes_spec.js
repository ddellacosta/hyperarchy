...
describe("Views.Pages.Meeting.Notes", function() {
  var member, meeting, note1, note2, creator1, creator2, notesRelation, notesView, longNoteBody;
  ...
  beforeEach(function() {
    ...
    note1 = meeting.notes().createFromRemote({id: 11, body: "I likeah the fruiloops so much", creatorId: creator1.id(), createdAt: 3245});
    note2 = meeting.notes().createFromRemote({id: 12, body: "Yez but sie koko krispies sind sehr yummy", creatorId: creator2.id(), createdAt: 3295});

    spyOn(MeetingNote.prototype, 'editableByCurrentUser').andReturn(true);

    notesView = Views.Pages.Meeting.Notes.toView();
    notesRelation = meeting.notes();
    ...
    describe("when the current user is a member", function() {
      describe("when the create note button is clicked", function() {
        it("clears and resizes the textarea, scrolls list to bottom, and submits a note to the server", function() {
          ...
          expect(createdRecord.meetingId()).toBe(meeting.id());
        });
      });
