_.constructor("Meeting", Model.Record, {
  constructorProperties: {
    initialize: function() {
      ...
      this.hasMany('notes', {constructorName: 'MeetingNote'});
      this.relatesToMany('noters', function() {
        return this.notes().join(User).on(MeetingNote.creatorId.eq(User.id));
      });
    ...
    updateScores: function() {
      ...
      this.each(function(meeting) {
        queue.add(meeting.hitch('updateScore'));
      });
      queue.start();
    }
  },
  ...
  currentUsersVisit: function() {
    return this.meetingVisits().find({userId: Application.currentUserId});
  },
  ...
  computeScore: function() {
    return (this.voteCount() + Meeting.SCORE_EXTRA_HOURS) / Math.pow(this.ageInHours() + Meeting.SCORE_EXTRA_HOURS, Meeting.SCORE_GRAVITY);
  },
