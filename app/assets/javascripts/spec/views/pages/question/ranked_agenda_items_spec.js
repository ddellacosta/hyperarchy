...
describe("Views.Pages.Question.RankedAgendaItems", function() {
  ...
  beforeEach(function() {
    ...
    ranking1 = currentUser.rankings().createFromRemote({id: 1, questionId: question.id(), agendaItemId: agendaItem1.id(), position: 64});
    ranking2 = currentUser.rankings().createFromRemote({id: 2, questionId: question.id(), agendaItemId: agendaItem2.id(), position: -64});
  ...
  describe("rankings", function() {
    ...
    it("clears the list when a new relation is assigned", function() {
      ...
      var agendaItemB = question.agendaItems().createFromRemote({id: 100, body: "AgendaItem B"});
  ...
  describe("drag and drop of rankings", function() {
    describe("sorting existing rankings", function() {
      ...
      beforeEach(function() {
        ...
        ranking3 = currentUser.rankings().createFromRemote({id: 3, questionId: question.id(), agendaItemId: agendaItem3.id(), position: -64});
        rankedAgendaItems.rankings(rankingsRelation);
    ...
    describe("receiving new rankings from the current consensus", function() {
      ...
      describe("when the current user is a member", function() {
        ...
        describe("when receiving a agendaItem that has not yet been ranked", function() {
          it("adds a new RankingLi for the agendaItem and associates it with a position", function() {
            var agendaItem3Li = questionPage.currentConsensus.find('li:contains("AgendaItem 3")');
            ...
            var ranking3 = Ranking.createFromRemote({id: 3, userId: currentUser.id(), agendaItemId: agendaItem3.id(), questionId: question.id(), position: 128});
            lastCreateOrUpdatePromise.triggerSuccess(ranking3);
        ...
        describe("when receiving a agendaItem that has already been ranked", function() {
          it("removes the previous RankingLi for the agendaItem and adds a new one, associating it with a position", function() {
            ...
            var agendaItem2Li = questionPage.currentConsensus.find('li:contains("AgendaItem 2")');
            var ranking1Li = rankedAgendaItems.find('li:contains("AgendaItem 1")');
      ...
      describe("when the user is a guest", function() {
        ...
        beforeEach(function() {
          ...
          agendaItem3Li = questionPage.currentConsensus.find('li:contains("AgendaItem 3")');
  ...
  describe("handling of remote events on rankings", function() {
    ...
    describe("when a ranking is inserted", function() {
      it("responds to a ranking inserted in the last positive position", function() {
        rankedAgendaItems.rankings(rankingsRelation);
        agendaItem3 = question.agendaItems().createFromRemote({id: 3, body: "AgendaItem 3"});
        ranking3 = currentUser.rankings().createFromRemote({id: 3, questionId: question.id(), agendaItemId: agendaItem3.id(), position: 8});
      ...
      it("responds to a ranking inserted in a positive position other than the last", function() {
        rankedAgendaItems.rankings(rankingsRelation);
        agendaItem3 = question.agendaItems().createFromRemote({id: 3, body: "AgendaItem 3"});
        ranking3 = currentUser.rankings().createFromRemote({id: 3, questionId: question.id(), agendaItemId: agendaItem3.id(), position: 128});
      ...
      it("responds to a ranking inserted in a negative position other than the last", function() {
        rankedAgendaItems.rankings(rankingsRelation);
        agendaItem3 = question.agendaItems().createFromRemote({id: 3, body: "AgendaItem 3"});
        ranking3 = currentUser.rankings().createFromRemote({id: 3, questionId: question.id(), agendaItemId: agendaItem3.id(), position: -32});
