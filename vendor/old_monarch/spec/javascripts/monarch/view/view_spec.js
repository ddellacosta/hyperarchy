  ...
  describe("Views constructed with OldMonarch.View.Template#toView", function() {
    ...
    describe("form interaction methods", function() {
      ...
      describe("#model(model)", function() {
        ...
        it("cancels previous update subscriptions when a new model is assigned", function() {
          view.model(model);
          view.model(SampleModel.created({foo: "new foo"}));
    ...
    describe("#registerInterest(object, methodNome, callback)", function() {
      ...
      it("subscribes to the given object using the specified method name, but only subscribes to this method on one object at a time and unsubscribes on destroy", function() {
        ...
        view.registerInterest(node1, 'subscribe', callback);
        expect(node1.size()).to(eq, 1);

        node1.publish("foo");
        expect(callback).to(haveBeenCalled, withArgs("foo"));
        expect(callback.mostRecentThisValue).to(eq, view);

        callback.clear();

        view.registerInterest(node2, 'subscribe', callback);
      ...
      it("allows a disambiguating name to be provided as a first argument, so that handlers can be registered on two different objects but the same method name", function() {
        ...
        view.registerInterest('a', node1, 'subscribe', callbackA);
        view.registerInterest('b', node2, 'subscribe', callbackB);
        expect(node1.size()).to(eq, 1);
        expect(node2.size()).to(eq, 1);

        node1.publish("foo");
        expect(callbackA).to(haveBeenCalled, withArgs("foo"));
        expect(callbackA.mostRecentThisValue).to(eq, view);
        expect(callbackB).toNot(haveBeenCalled);

        callbackA.clear();

        node2.publish("quux");
        expect(callbackB).to(haveBeenCalled, withArgs("quux"));
        expect(callbackB.mostRecentThisValue).to(eq, view);
        expect(callbackA).toNot(haveBeenCalled);

        callbackB.clear();

        view.registerInterest('a', node3, 'subscribe', callbackA);
        expect(node1.size()).to(eq, 0);
        expect(node2.size()).to(eq, 1);
        ...
        node3.publish("baz");
        expect(callbackA).to(haveBeenCalled, withArgs("baz"));
        expect(callbackA.mostRecentThisValue).to(eq, view);
        expect(callbackB).toNot(haveBeenCalled);
        ...
        expect(node1.size()).to(eq, 0);
        expect(node2.size()).to(eq, 0);
        expect(node3.size()).to(eq, 0);

        node1.publish("larp");
        node2.publish("zoop!");
        node3.publish("yehaaaa!");
        expect(callbackA).toNot(haveBeenCalled);
        expect(callbackB).toNot(haveBeenCalled);
      });
    });
