...
Screw.Unit(function(c) { with(c) {
  describe("OldMonarch.Http.Server", function() {
    ...
    describe("#fetch", function() {
      ...
      it("if passed an empty array, triggers success on the promise immediately without requesting to the server", function() {
        var promise = server.fetch([]);
        expect(requests.length).to(eq, 0);
        var successHandler = mockFunction('successHandler')
        promise.success(successHandler);
        expect(successHandler).to(haveBeenCalled);
      });
    });
