...
Screw.Unit(function(c) { with(c) {
  describe("OldMonarch.Http.AjaxFuture", function() {
    var future;

    before(function() {
      future = new OldMonarch.Http.AjaxFuture();
    });
      ...
      context("when the 'successful' key is true in the given response", function() {
        ...
        context("when the response also includes records", function() {
          it("invokes success callbacks with the response's 'data' hash after updating the repository with the dataset and does not invoke failure callbacks", function() {
            ...
            future.success(successCallback);
    ...
    describe("#success and #onFailure", function() {
      they("return the future object for method chaining", function() {
        expect(future.success(mockFunction())).to(eq, future);
        expect(future.onFailure(mockFunction())).to(eq, future);
      });
      ...
      context("when #handleResponse has already been called with a successful response", function() {
        ...
        it("invokes an #success callback immediately with the response's data", function() {
          var successCallback = mockFunction("success callback");
          future.success(successCallback);
          expect(successCallback).to(haveBeenCalled, withArgs(data));
        });
      ...
      context("when #handleResponse has already been called with an unsuccessful response", function() {
        ...
        it("does not invoke an #success callback", function() {
          var successCallback = mockFunction("success callback");
          future.success(successCallback);
          expect(successCallback).toNot(haveBeenCalled);
        });
