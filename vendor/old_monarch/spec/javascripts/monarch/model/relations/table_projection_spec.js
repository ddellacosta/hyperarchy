...
Screw.Unit(function(c) { with(c) {
  describe("OldMonarch.Model.Relations.TableProjection", function() {
    ...
    init(function() {
      user = User.created({id: 'saltpeter', fullName: "Salt Peter"});
      blog1 = user.blogs().created({id: 1, name: "Blog 1", ownerId: 1});
      blog2 = user.blogs().created({id: 2, name: "Blog 2", ownerId: 2});
      post1 = blog1.blogPosts().created({id: 2, body: "this is post 1"});
      post2 = blog1.blogPosts().created({id: 3, body: "this is post 2"});
      post3 = blog2.blogPosts().created({id: 1, body: "this is post 3"});

      operand = BlogPost.table.join(user.blogs()).on(BlogPost.blogId.eq(Blog.id));
      ...
      describe("when a tuple is inserted into the operand", function() {
        context("if the record corresponding to the projected table is NOT already present in the projection", function() {
          it("triggers an insert event with the record", function() {
            ...
            var post = blog.blogPosts().created({id: 1, blogId: 100});
