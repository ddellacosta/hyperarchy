  ...
  describe("OldMonarch.Model.Relations.Table", function() {
    ...
    describe("column definition", function() {
      ...
      describe("#column(name)", function() {
        it("returns the concrete or synthetic column with the given name or null if none exists", function() {
    ...
    describe("#findOrFetch(id, additionalRelations)", function() {
      ...
      it("looks for a record in with the given id, and fetches it and any additional relations if it is not found, invoking the callback with the record and a boolean value indicating whether a fetch occurred", function() {
        ...
        expect(successCallback).to(haveBeenCalled, withArgs(extantRecord, false));
        ...
        expect(Server.fetches.length).to(eq, 1);
        expect(Server.lastFetch.relations).to(equal, [Blog.where({id: 1}), additionalRelation]);

        Server.lastFetch.simulateSuccess({
          blogs: {
            '1': { id: 1, name: "Fetched From The Server"}
          }
        });

        expect(Blog.find(1)).toNot(beUndefined);

        expect(successCallback).to(haveBeenCalled, withArgs(Blog.find(1), true));
      });

      it("invokes invalid callbacks if a record is still not found after attempting a fetch", function() {
        var successCallback = mockFunction("successCallback");
        var invalidCallback = mockFunction("invalidCallback");
        Blog.findOrFetch(1).success(successCallback).invalid(invalidCallback);

        Server.lastFetch.simulateSuccess({});

        expect(successCallback).toNot(haveBeenCalled);
        expect(invalidCallback).to(haveBeenCalled);
      });
    });
