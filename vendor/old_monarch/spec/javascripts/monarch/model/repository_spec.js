...
Screw.Unit(function(c) { with(c) {
  describe("OldMonarch.Model.Repository", function() {
    useLocalFixtures();
    ...
    describe("#mutate", function() {
      ...
      it("also updates the repository with associated records included with the commands", function() {
        var insertCallback = mockFunction('insertCallback', function() {
          expect(User.find(101)).toNot(beUndefined);
        });
        var updateCallback = mockFunction('updateCallback', function() {
          expect(User.find(102)).toNot(beUndefined);
        });
        var removeCallback = mockFunction('removeCallback', function() {
          expect(User.find(103)).toNot(beUndefined);
        });
        ...
        repository.mutate([
          ['create', 'blogs', { id: "malathion", name: "Recipes From The Makers of Malathion", user_id: 99}, { users: { 101: { id: 101, fullName: "Guy 1" }}}],
          ['update', 'users', 'jan', { age: 88 }, { users: { 102: { id: 102, fullName: "Guy 2" }}}],
          ['destroy', 'users', 'wil', { users: { 103: { id: 103, fullName: "Guy 3" }}}],
        ]);
