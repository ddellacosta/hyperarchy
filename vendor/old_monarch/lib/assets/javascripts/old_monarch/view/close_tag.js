  ...
  bind: function() {
    ...
    this.onBuild(function(element, view) {
      var outerHandler = function(event, completionFunctionOrHash) {
        ...
        var resultOrPromise = innerHandler.call(view, event, element);
        if (completionFunctionOrHash) {
          if (!resultOrPromise) throw new Error("You must return a promise from this event handler: " + innerHandler.toString());

          if (_.isFunction(completionFunctionOrHash)) {
            resultOrPromise.success(completionFunctionOrHash);
          } else {
            _.each(completionFunctionOrHash, function(callback, eventType) {
              resultOrPromise[eventType](callback);
            });
          }
        }
